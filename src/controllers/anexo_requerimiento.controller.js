...
const updateAnexoRequerimiento = async (req, res) => {
  try {
    console.log("ok upload");
    await Anexo.deleteAllAnexosForRequerimiento(req.body.id_requerimiento);

    const promises = req.files.map((file) => {
      return Anexo.updateAnexoRequerimiento(
        file.filename,
        path.extname(file.filename).substring(1),
        req.body.id_requerimiento
      );
